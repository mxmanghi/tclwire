...
#
# This module converts normalized runtime configuration into scheduler chore
# specs. Server chores are singleton runtime tasks. Application chores are
# per-application tasks and carry an application_context dictionary containing
# application_id, serialized ApplicationConfiguration, and TPBA pool_key for
# classes that opt into ::tclwire::ApplicationChore.
...
namespace eval ::tclwire::runtime {
    ...
    proc config_has_server_chores {config} {
        return [expr {[dict exists $config server_chores] &&
                [llength [dict get $config server_chores]] > 0}]
    }
